...
Génère ${"{count}"} entreprises françaises RÉELLES correspondant aux critères. Pour chaque compte, produis une analyse commerciale de niveau expert — celle qu'un directeur commercial donnerait à un SD avant un appel découverte.
...
{"leads":[{
  ...
  "dirigeant": "Prénom Nom — PDG/DG si connu",

  "contact_cible": "Poste exact à contacter en premier (DSI / DA / Directeur R&D / DRH...)",
  "champion_interne": "Poste de l'allié opérationnel probable (diff. du décideur budget)",
  "economic_buyer": "Qui signe réellement le bon de commande",

  "signal": "Signal d'intention TRÈS CONCRET — fait précis, daté si possible",
  "signal_force": 4,
  "source_signal": "Où exactement détecter ce signal (LinkedIn / APEC / Presse / BODACC...)",

  "meddic": {
    "metrics": true,
    "metrics_why": "Pourquoi ce critère est validé ou non, en 1 phrase précise",
    "economic_buyer": true,
  ...
  "tjm_marche": "TJM marché pour le profil cible chez ce compte, ex: 700–850€/j (Paris)",
  "potentiel_ca": "CA annuel estimé si 1-3 missions ouvertes, ex: 180–450k€/an",
  "angle_consultime": "Quel argument Consultime mettre EN PREMIER pour ce compte (vivier/vitesse/CANUT/référence/périmètre...)",
  "risque_concurrent": "Risque concurrentiel principal et comment le gérer",
  "type_contrat": "Direct | Accord-cadre | AO | CANUT",
  "timing_optimal": "Meilleur moment pour contacter (cycle budgétaire, période, événement déclencheur)",
  "profil_type": "Profil freelance le plus probable à placer (ex: Chef de projet SI senior, Data Engineer, Architecte cloud...)",

  "questions_qualification": [
    "Q1 — question MEDDIC spécifique à CE compte, pas générique",
    "Q2 — question budget/décision",
    "Q3 — question urgence/timing"
  ],

  "accroche_linkedin": "5 lignes MAX. Ouvrir sur UN fait concret sur cette entreprise (signal ou contexte). Proposer de la valeur. CTA simple. Écrire 'Consultime'. JAMAIS FI/FE/FC. JAMAIS portage salarial. JAMAIS formule de politesse.",
  "accroche_email": {
    "objet": "≤8 mots, percutant, personnalisé",
    "corps": "≤100 mots. Différent du LinkedIn. Même ton direct. Référence sectorielle si pertinent."
  },

  "plateforme": "FI",
  "priorite": 1
}],"resume":"2 phrases : pourquoi ces comptes, potentiel global estimé.","total_ca_potentiel":"ex: 800k–2M€/an"}

RÈGLES ABSOLUES :
- signal_force : 1 (faible) à 5 (besoin immédiat confirmé)
- CHAUD = 3 critères MEDDIC vrais, TIEDE = 2, FROID ≤ 1
- plateforme : FI (dev/data/cloud/archi/cyber/PM/QA/UX) | FE (ingénierie/R&D/HSE/mécanique) | FC (MOA/PMO/finance/RH/supply/contrôle de gestion)
- priorite : 1 (contacter cette semaine), 2 (ce mois), 3 (veille)
- Références UNIQUEMENT du même secteur : MAIF/MACIF/COVEA/Europ Assistance (assurance), EDF/RTE/GRDF (énergie), BPCE/Natixis/Banque de France (finance), RATP/SNCF (transport), Nexans/Bonduelle/Saint-Gobain (industrie), ACOSS/AFPA/CNAM (public)
- Correction géo automatique : Lyon/Bordeaux/Toulouse → TJM -10 à -15% vs Paris
- type_contrat CANUT uniquement pour secteur public/opérateurs publics
- Ne jamais citer Malt comme concurrent — toujours "complémentaire"`;

const SEQ_SYS = `Tu es directeur commercial senior Consultime. Génère une séquence de prospection B2B J0–J21 chirurgicalement personnalisée pour ce compte.

Chaque message doit contenir 1 élément de personnalisation contextuelle spécifique à CE compte. Pas de générique. Jamais FI/FE/FC. Jamais portage salarial. Toujours sortir d'un appel avec une prochaine action datée.

RETOURNE UNIQUEMENT JSON sans markdown :
{
  "sequence": [
    {"jour":0,"canal":"LinkedIn","action":"Demande de connexion","message":"1 phrase contextuelle uniquement — pas de pitch"},
    {"jour":3,"canal":"LinkedIn","action":"1er message","message":"5 lignes MAX. Fait concret sur l'entreprise. Valeur. CTA simple."},
    {"jour":7,"canal":"Email","action":"Cold email","objet":"≤8 mots","message":"≤100 mots. Différent du LinkedIn. Référence sectorielle."},
    {"jour":12,"canal":"LinkedIn","action":"Relance valeur","message":"3 lignes. Apporter quelque chose (profil dispo / benchmark TJM / insight marché). Pas de relance sèche."},
    {"jour":16,"canal":"Téléphone","action":"Appel découverte","message":"Script 30 secondes mot pour mot. Accroche → douleur → valeur → demande de RDV daté."},
    {"jour":21,"canal":"Email","action":"Dernier contact","message":"Bienveillant. Une seule question ouverte. Fermeture propre sans agressivité."}
  ],
  "objections": [
    {"objection":"Objection la plus probable pour CE secteur et CE profil de contact","reponse":"Script mot pour mot — utiliser les arguments Consultime adaptés à ce compte","question_retour":"Question de retour pour requalifier"},
    {"objection":"On travaille déjà avec Malt","reponse":"Malt et Consultime sont complémentaires — [développer avec angle spécifique à ce compte]","question_retour":"Question retour"},
    {"objection":"Pas de budget / pas de besoin en ce moment","reponse":"Script Consultime mot pour mot","question_retour":"Question retour"},
    {"objection":"Trop chers","reponse":"Par rapport à quoi exactement ? [ancrage TJM marché spécifique à ce profil]","question_retour":"Ancrage prix"}
  ],
  "post_rdv": {
    "j1_objet": "Objet email J+1",
    "j1_corps": "Récap douleurs identifiées + recommandation profil + prochaine étape datée des deux côtés",
    "j7_sans_reponse": "1 contenu à valeur (profil disponible / TJM marché / retour client même secteur) + CTA simple non intrusif"
  },
  "disqualification": "Critères précis pour passer ce compte en veille et ne plus relancer"
}`;
...
function repairJSON(raw) {
  ...
  if (li > -1) {
    ...
    if (as > -1) {
      ...
      if (valid.length > 0) return JSON.parse(`{"leads":[${valid.join(",")}],"resume":"Résultats partiels — réponse tronquée.","total_ca_potentiel":"N/C"}`);
    }
  }
...
function sortLeads(leads, by) {
  ...
  if (by === "ca")       return copy.sort((a, b) => {
    const parse = s => parseInt((s || "0").replace(/[^0-9]/g, "")) || 0;
    return parse(b.potentiel_ca) - parse(a.potentiel_ca);
  });
...
function MeddicDot({ label, active, color, why }) {
  return (
    <div title={`${label}${why ? ` — ${why}` : ""}`} style={{ width: 22, height: 22, background: active ? color : P.bg, border: `1px solid ${active ? color : P.border}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontFamily: P.mono, color: active ? "#fff" : P.tf, fontWeight: 700, cursor: "help" }}>
      {label[0]}
    </div>
  );
}
...
function SignalBar({ force }) {
  return (
    <div style={{ display: "flex", gap: 2, alignItems: "center" }} title={`Force signal : ${force}/5`}>
      {[1, 2, 3, 4, 5].map(i => (
        <div key={i} style={{ width: 6, height: 10, background: i <= force ? (force >= 4 ? P.g : force >= 3 ? P.o : P.r) : P.border }} />
      ))}
    </div>
  );
}
...
function Toast({ toasts }) {
  return (
    <div style={{ position: "fixed", bottom: 24, right: 24, display: "flex", flexDirection: "column", gap: 8, zIndex: 100 }}>
      {toasts.map(t => (
        <div key={t.id} style={{ background: t.type === "error" ? P.r : P.g, color: "#fff", padding: "10px 16px", fontSize: 12, fontFamily: P.mono, letterSpacing: "0.06em", boxShadow: "0 4px 12px rgba(0,0,0,0.15)", animation: "slideIn 0.2s ease" }}>
          {t.msg}
        </div>
      ))}
    </div>
...
export default function App() {
  ...
  const cp = (text, id) => { navigator.clipboard.writeText(text); setCopied(id); toast("Copié dans le presse-papiers"); setTimeout(() => setCopied(null), 2000); };
  ...
  const doSearch = useCallback(async (overrideQuery = null) => {
    ...
    const q = overrideQuery || [
      ...
      size && `Taille : ${size}`,
      kw && `Mots-clés / signal : ${kw}`,
    ].filter(Boolean).join("\n");

    const systemWithCount = SEARCH_SYS.replace("${\"count\"}", String(count));

    try {
      setStep("Analyse du marché et des signaux d'intention…");
      const r = await callAPI(systemWithCount, q, 8000);
      setResults(r.leads || []);
      setResume(r.resume || "");
      setTotalCA(r.total_ca_potentiel || "");

      // Sauvegarder dans l'historique
      if (!overrideQuery) {
        const entry = { q, sector, region, size, kw, count, at: new Date().toISOString() };
        const newHistory = [entry, ...history.filter(h => h.q !== q)].slice(0, MAX_HISTORY);
  ...
  const saveLead = (lead) => {
    ...
    setPipeline(u); saveLS(PIPELINE_KEY, u);
    setSaved(s => new Set([...s, lead.nom]));
    toast(`${lead.nom} ajouté au pipeline`);
  };
  ...
  const delLead   = (id)    => { const u = pipeline.filter(l => l._id !== id); setPipeline(u); saveLS(PIPELINE_KEY, u); toast("Lead retiré du pipeline"); };
...
Contact cible : ${lead.contact_cible} | Champion interne : ${lead.champion_interne || "N/A"} | Economic Buyer : ${lead.economic_buyer || "N/A"}
Signal : ${lead.signal} (force : ${lead.signal_force}/5)
MEDDIC : ${lead.meddic?.global} | Plateforme : ${lead.plateforme} | TJM marché : ${lead.tjm_marche}
Angle Consultime : ${lead.angle_consultime} | Type contrat : ${lead.type_contrat} | Timing : ${lead.timing_optimal}
Profil type : ${lead.profil_type} | Risque concurrent : ${lead.risque_concurrent}`;
      const r = await callAPI(SEQ_SYS, ctx, 6000, true);
      setSeqs(s => ({ ...s, [key]: r }));
      toast("Séquence générée");
    } catch (e) {
      setSeqs(s => ({ ...s, [key]: { error: e.message } }));
      toast(e.message, "error");
    }
    setLoadSeq(null);
  };
  ...
  const exportCSV = (data, filename) => {
    const h = ["Nom","Secteur","CA","Effectif","Siège","Dirigeant","Contact cible","Champion","Buyer","Signal","Force signal","Source","MEDDIC","TJM marché","CA potentiel","Angle","Type contrat","Timing","Profil type","Plateforme","Priorité","Statut","Accroche LI","Email Objet","Email Corps"];
    const rows = data.map(l => [
      l.nom, l.secteur, l.ca_estime, l.effectif, l.siege, l.dirigeant||"",
      l.contact_cible, l.champion_interne||"", l.economic_buyer||"",
      l.signal, l.signal_force||"", l.source_signal,
      l.meddic?.global, l.tjm_marche||"", l.potentiel_ca||"",
      l.angle_consultime||"", l.type_contrat||"", l.timing_optimal||"", l.profil_type||"",
      l.plateforme, l.priorite||"", l._status||"",
    ...
    a.download = filename || `ct_prospects_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    return (
      <div style={{ background: P.card, border: `1px solid ${P.border}`, borderLeft: `4px solid ${col}`, boxShadow: "0 1px 5px rgba(0,0,0,0.04)" }}>
        ...
        <div style={{ padding: "14px 18px 12px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", gap: 8, fontFamily: P.mono, fontSize: 10, color: P.tmt, flexWrap: "wrap", marginBottom: 5 }}>
              {[lead.secteur, lead.ca_estime, lead.effectif, lead.siege].filter(Boolean).reduce((a, v, i, arr) => [...a, v, ...(i < arr.length - 1 ? [<span key={`d-${i}`} style={{ color: P.border }}>·</span>] : [])], []).map((v, i) => <span key={i}>{v}</span>)}
            </div>
            {/* Contact cible */}
          ...
          <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
            ...
            {!isPL && (
              <button onClick={() => saveLead(lead)} disabled={isSaved} style={{ background: isSaved ? P.gb : P.surface, border: `1px solid ${isSaved ? P.g : P.bdk}`, color: isSaved ? P.g : P.tm, padding: "4px 10px", fontSize: 9, fontFamily: P.mono, cursor: isSaved ? "default" : "pointer", textTransform: "uppercase", whiteSpace: "nowrap" }}>
        ...
        <div style={{ padding: "8px 18px", borderTop: `1px solid ${P.border}`, display: "flex", gap: 16, flexWrap: "wrap" }}>
          {lead.tjm_marche && (
            <div>
              <div style={{ fontFamily: P.mono, fontSize: 8, color: P.tmt, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>TJM marché</div>
              <div style={{ fontFamily: P.mono, fontSize: 11, fontWeight: 600, color: P.t }}>{lead.tjm_marche}</div>
            </div>
          )}
          {lead.potentiel_ca && (
            <div>
              <div style={{ fontFamily: P.mono, fontSize: 8, color: P.tmt, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>CA potentiel</div>
              <div style={{ fontFamily: P.mono, fontSize: 11, fontWeight: 600, color: P.g }}>{lead.potentiel_ca}</div>
            </div>
          )}
          {lead.profil_type && (
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: P.mono, fontSize: 8, color: P.tmt, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>Profil cible</div>
              <div style={{ fontSize: 11, color: P.tm }}>{lead.profil_type}</div>
            </div>
          )}
          {lead.timing_optimal && (
            <div style={{ flexShrink: 0 }}>
              <div style={{ fontFamily: P.mono, fontSize: 8, color: P.tmt, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>Timing</div>
              <div style={{ fontSize: 11, color: P.tm }}>{lead.timing_optimal}</div>
            </div>
          )}
        </div>
        ...
        {/* ── Pipeline notes ── */}
        {isPL && (
          <div style={{ padding: "6px 18px", borderTop: `1px solid ${P.border}` }}>
            <textarea value={lead._notes || ""} onChange={e => updNotes(lead._id, e.target.value)} placeholder="Notes…" style={{ width: "100%", background: "transparent", border: "none", outline: "none", color: P.tm, fontSize: 11, fontFamily: P.sans, resize: "none", lineHeight: 1.5, boxSizing: "border-box", minHeight: 28 }} />
          </div>
        )}
            ...
            <div style={{ borderTop: `1px solid ${P.border}` }}>
              ...
              <div style={{ padding: "16px 18px" }}>
                ...
                {tab === "radar" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {[["M — Metrics", m.metrics, m.metrics_why], ["E — Economic Buyer", m.economic_buyer, m.economic_buyer_why], ["I — Identify Pain", m.identify_pain, m.identify_pain_why]].map(([label, active, why]) => (
                          <div key={label} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 12px", background: active ? P.gb : P.rb, border: `1px solid ${active ? P.g + "44" : P.r + "44"}` }}>
                            ...
                            <div>
                              <div style={{ fontFamily: P.mono, fontSize: 10, color: active ? P.g : P.r, fontWeight: 600, marginBottom: 2 }}>{label}</div>
                              <div style={{ fontSize: 12, color: P.tm, lineHeight: 1.5 }}>{why || "—"}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {lead.questions_qualification?.length > 0 && (
                      <div>
                        <Lbl>Questions de qualification (MEDDIC)</Lbl>
                        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                ...
                {tab === "sequence" && (
                  <>
                    {!seq && loadSeq !== ckey && (
                      <button onClick={() => genSeq(lead, ckey)} style={{ background: P.a, border: "none", color: "#fff", padding: "10px 20px", fontSize: 11, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: P.sans }}>
                        Générer la séquence J0–J21
                      </button>
                    )}
                    {loadSeq === ckey && (
                      <div style={{ display: "flex", alignItems: "center", gap: 10, color: P.tmt, fontFamily: P.mono, fontSize: 11 }}>
                        <div style={{ width: 16, height: 16, border: `2px solid ${P.border}`, borderTopColor: P.a, borderRadius: "50%", animation: "spin 0.75s linear infinite" }} />
                        Génération de la séquence personnalisée…
                      </div>
                    )}
                    {seq && !seq.error && (
                      <>
                            ...
                            <div key={si} style={{ background: P.bg, border: `1px solid ${P.border}`, padding: "10px 14px" }}>
                              <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
                                ...
                                <div style={{ marginLeft: "auto" }}>
                                  <CopyBtn text={s.objet ? `Objet : ${s.objet}\n\n${s.message}` : s.message} id={`sq-${ckey}-${si}`} copied={copied} onCopy={cp} />
                                </div>
                              </div>
                        ...
                        {/* Post-RDV + Disqualification */}
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                          {seq.post_rdv?.j1_corps && (
                            <div style={{ background: P.bg, border: `1px solid ${P.border}`, padding: "10px 14px" }}>
                              <Lbl>J+1 post-RDV</Lbl>
                              <div style={{ fontFamily: P.sans, fontSize: 11, fontWeight: 600, color: P.t, marginBottom: 4 }}>Objet : {seq.post_rdv.j1_objet}</div>
                              <div style={{ fontSize: 11, color: P.tm, lineHeight: 1.5 }}>{seq.post_rdv.j1_corps}</div>
                            </div>
                          )}
                          {seq.disqualification && (
                            <div style={{ background: P.rb, border: `1px solid ${P.r}33`, padding: "10px 14px" }}>
                              <Lbl>Critères de disqualification</Lbl>
                              <div style={{ fontSize: 11, color: P.tm, lineHeight: 1.5 }}>{seq.disqualification}</div>
                            </div>
                          )}
                        </div>
                      </>
        ...
        {isPL && (
          <div style={{ borderTop: `1px solid ${P.border}`, padding: "5px 18px", display: "flex", justifyContent: "flex-end" }}>
            <button onClick={() => delLead(lead._id)} style={{ background: "transparent", border: "none", color: P.tf, fontSize: 10, fontFamily: P.mono, cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.color = P.r} onMouseLeave={e => e.currentTarget.style.color = P.tf}>× Retirer</button>
          </div>
        )}
      </div>
    );
  ...
  const plStats = {
    ...
    chaud: pipeline.filter(l => l.meddic?.global === "CHAUD").length,
    rdv: pipeline.filter(l => l._status === "meeting").length,
    qualified: pipeline.filter(l => l._status === "qualified").length,
  };
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        body{background:${P.bg};font-family:${P.sans}}
        @keyframes spin{to{transform:rotate(360deg)}}
        @keyframes slideIn{from{opacity:0;transform:translateX(20px)}to{opacity:1;transform:translateX(0)}}
        @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}
        textarea::placeholder,input::placeholder{color:${P.tf}}
        select option{background:${P.surface};color:${P.t}}
        ::-webkit-scrollbar{width:4px;height:4px}
        ::-webkit-scrollbar-thumb{background:${P.bdk}}
        .gh:hover{background:${P.ab}!important;border-color:${P.a}!important;color:${P.a}!important}
      `}</style>

      {/* Progress bar */}
      {progress > 0 && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, height: 3, zIndex: 200 }}>
          <div style={{ height: "100%", width: `${progress}%`, background: P.a, transition: "width 0.4s ease" }} />
        </div>
      )}
      ...
      <div style={{ background: P.surface, borderBottom: `1px solid ${P.border}`, padding: "13px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {pipeline.length > 0 && <button className="gh" onClick={() => exportCSV(pipeline, `pipeline_${new Date().toISOString().slice(0, 10)}.csv`)} style={{ background: P.surface, border: `1px solid ${P.bdk}`, color: P.tm, padding: "6px 12px", fontFamily: P.mono, fontSize: 9, letterSpacing: "0.1em", cursor: "pointer", textTransform: "uppercase" }}>↓ Pipeline CSV</button>}
        </div>
      </div>
      ...
      {view === "search" && (
        <div style={{ display: "flex", maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ width: 272, flexShrink: 0, padding: "20px 16px", borderRight: `1px solid ${P.border}`, minHeight: "calc(100vh - 100px)", display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              <Lbl>Mots-clés / signal détecté</Lbl>
              <input value={kw} onChange={e => setKw(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) doSearch(); }} placeholder="Ex : recrutement IT gelé, ERP en cours, RSSI recruté…" style={{ width: "100%", background: P.surface, border: `1px solid ${P.border}`, color: P.t, padding: "9px 11px", fontSize: 12, fontFamily: P.sans, outline: "none", boxSizing: "border-box" }} />
            </div>
            ...
            <button onClick={() => doSearch()} disabled={!canSearch} style={{ background: canSearch ? P.a : P.bdk, border: "none", color: canSearch ? "#fff" : P.tf, padding: "12px 20px", fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", cursor: canSearch ? "pointer" : "not-allowed", fontFamily: P.sans, marginTop: 4 }}>
              {loading ? "Analyse en cours…" : "Trouver des prospects ⌘↵"}
            </button>

            {results.length > 0 && (
              <button className="gh" onClick={() => exportCSV(results)} style={{ background: P.surface, border: `1px solid ${P.bdk}`, color: P.tm, padding: "8px 12px", fontSize: 9, fontFamily: P.mono, letterSpacing: "0.1em", cursor: "pointer", textTransform: "uppercase" }}>
                ↓ Export résultats ({results.length})
              </button>
            )}
          ...
          <div style={{ flex: 1, padding: "20px", minWidth: 0 }}>
            {loading && (
              <div style={{ marginBottom: 20 }}>
                <div style={{ textAlign: "center", padding: "32px 0 24px" }}>
                  <div style={{ display: "inline-block", width: 26, height: 26, border: `2px solid ${P.border}`, borderTopColor: P.a, borderRadius: "50%", animation: "spin 0.75s linear infinite", marginBottom: 16 }} />
                  <div style={{ fontFamily: P.mono, fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: P.tm, marginBottom: 6 }}>{step}</div>
                </div>
            ...
            {resume && (
              <div style={{ background: P.ab, border: `1px solid ${P.a}33`, borderLeft: `3px solid ${P.a}`, padding: "12px 18px", marginBottom: 16, fontSize: 13, lineHeight: 1.65, color: P.tm }}>
                <span style={{ fontFamily: P.mono, fontSize: 9, color: P.a, letterSpacing: "0.14em", textTransform: "uppercase", marginRight: 10 }}>SYNTHÈSE</span>{resume}
              </div>
            )}
            ...
            {!loading && results.length === 0 && !error && (
              <div style={{ textAlign: "center", padding: "80px 0", color: P.tf }}>
                <div style={{ fontSize: 48, marginBottom: 14, opacity: 0.25 }}>⌖</div>
                <div style={{ fontFamily: P.mono, fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: P.tmt }}>Aucune recherche lancée</div>
                <div style={{ fontFamily: P.mono, fontSize: 9, color: P.tf, marginTop: 8 }}>Renseigne tes critères et lance la recherche (⌘↵)</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "20px 24px" }}>
          ...
          {pipeline.length > 0 && (
            <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
              {[
                { label: "Total", value: plStats.total, color: P.t },
                { label: "Chaud MEDDIC", value: plStats.chaud, color: P.g },
                { label: "RDV planifiés", value: plStats.rdv, color: P.te },
                { label: "Qualifiés", value: plStats.qualified, color: P.g },
